...
In addition to indicating which folder includes our models in the `dbt_project.yml` file, we can create .yml files in our _models/_ directory (as defined by the `model-paths` config) to set the model properties. You can name these files _whatever_you_want.yml_, and nest them arbitrarily deeply in subfolders within the _models/_ directory. One example for a .yml file in the _models/_ directory is the file called schema.yml, which is created in the default folder structure, after we initialize a dbt project with `dbt init`.
...
Here is an example to understand how many different model properties we can set:
...
models:
  ...
  - name: order_summary
    ...
    config:
      materialized: table
      # Below line will create a schema called `raw_analytics`. This is because we have set `schema: raw` in the `profiles.yml` file. This is provided as an example
      schema: "analytics"
      tags: ["orders", "summary"]
    columns:
      - name: customer_id
  ...
  - name: daily_orders
    ...
    columns:
      ...
      - name: average_order_value
        description: "Average value of orders placed on that date"
```

In the above example .yml file, we have `tags` as well. We are not going to use these but they could be useful to tag specific models and run those models. Here are example commands using `tags`:

```sh
# Run only models with 'orders' tag
dbt run --select tag:orders

# Run models with multiple tags
dbt run --select tag:orders tag:monthly

# Run all models except those with 'deprecated' tag
dbt run --exclude tag:deprecated

# Run models with 'analytics' tag and their dependencies
...
Sources are defined in .yml files nested under a `sources:` key. The `sources` could be in the same file as the model properties but it's not recommended. With `sources` we provide the data (db, schema, tables) to be transformed. Declaring these tables as sources in dbt, helps to

- select from source tables in your models using the `{{ source() }}` function
- test your assumptions about your source data
- calculate the freshness of your source data For example,
...
sources:
  - name: raw_data # This is the "Source Name" used in {{ source() }}
    schema: raw # This tells dbt to look in the 'raw' schema in Postgres
    tables:
      - name: orders # This is the actual table name in your database
...
We can have more than one source in the source .yml file:
...
customer_segments as (
  SELECT
    ...
    last_order_date,
    customer_tier
  FROM {{ ref('order_summary') }} -- Referencing your previous model
)

-- I'll join them here so you can see both source and ref working together
SELECT
    cs.customer_tier,
...
Once that generic test has been defined, it can be added as a property on any existing model (or source, seed, or snapshot). These properties are added in .yml files in the same directory as your resource.
...
Now we can use `is_positive` just like you used `unique` or `not_null`.
...
Now, we can specifically run the test with the below command:

```sh
dbt test --select order_summary
```

Let's now write a generic test that accepts arguments. In the previous example, we only used the implicit arguments. Every generic test automatically receives two arguments from dbt: `model` and `column_name`. When we wrote - `is_positive` in the YAML, dbt looked at which column it was under (`lifetime_value`) and which model it belonged to (`order_summary`) and passed those values into the macro automatically. We will add a third argument called `threshold`. We will also give it a default value of `0`, so that if we don't provide one, it behaves like our previous "positive" test.
...
There is an additional type of test to dbt - unit tests. In software programming, unit tests validate small portions of your functional code, and they work much the same way here. Unit tests allow you to validate your SQL modeling logic on a small set of static inputs before you materialize your full model in production.
